...
#define OV9281_GAIN_DEFAULT		0x30    /* See https://docs.google.com/document/d/1A1HwLcnAiAV0Wq2KKsoAjFqxJn9qE0hUOO_BfR2pvEU/edit */
...
static int ov9281_s_stream(struct v4l2_subdev *sd, int on)
{
	...
	if (on) {
		/*
		 * If streaming is ON then stop and restart stream
		 * This is done so streaming won't be blocked by past failures to close the stream (STREAMOFF)
		 */
		if (ov9281->streaming) {
			/* Stop Stream */
			ret = __ov9281_stop_stream(ov9281);
			pm_runtime_put_noidle(&client->dev);
			if (ret) {
				v4l2_err(sd, "stop stream failed while write regs\n");
...
static int ov9281_g_register(struct v4l2_subdev *sd, struct v4l2_dbg_register *dbg)
{
        struct ov9281 *ov9281 = to_ov9281(sd);
        struct i2c_client *client = ov9281->client;
        /* There are manual type conversions required. The register is only 8-bit, but ov7251_read_reg expects u32, and dbg->val is u64. */
        u32 val;

        int ret = ov9281_read_reg(client, dbg->reg, OV9281_REG_VALUE_08BIT, &val);

        if(ret < 0) return -EIO;

        dbg->val = val;
        dbg->size = 1;  /* We assume it's a 1-byte register. */

        return 0;
}

static struct regval shutter_registers[] = {
    {0x3500, 0x0},
    {0x3501, 0x0},
    {0x3502, 0x0},
    {REG_NULL, 0x00}    /* Sentinel indicating end of array. */
};

static int ov9281_s_register(struct v4l2_subdev *sd, const struct v4l2_dbg_register *dbg)
{
    /* Per https://www.kernel.org/doc/html/v4.9/media/uapi/v4l/vidioc-dbg-g-register.html, writing a register need not
       initialize the size. I guess we assume that the driver knows the register size.
       We only support 8-bit registers. */
    struct ov9281 *ov9281 = to_ov9281(sd);
    struct i2c_client *client = ov9281->client;
    ...
    /*
     * AR 2021-11-11: HACK alert!
     * We were seeing major artifacts when setting the shutter caused by register tearing and setting the registers at random times
     * (as opposed to during a vertical sync).
     * Those are both fixable using the group write system of the OV9281.
     * That's not the hack. The hack is that normally you would use the control interface to set the shutter.
     * But for whatever reason, I can't get the control interface hooked up properly, so we go through the read/write register system,
     * even though that's really supposed to just be for debugging.
     * But in order to correctly write the registers in a synchronized manner, I RELY ON THE REGSTERS BEING WRITTEN such that
     * 0x3502 is written LAST. Only when that happens do I actually send across the registers.
     * Note that before I found the group write functionality I experimented with the ov9281_write_array function, hoping that that
     * would be atomic, and then with ov9281_write_reg passing three registers in for the same reason, but neither were.
     * That's why the code is structured this way.
     * The group write is what ultimately fixed things.
     */

    if(dbg->reg == shutter_registers[0].addr) {
        shutter_registers[0].val = dbg->val;
        return 0;
    }
    else if(dbg->reg == shutter_registers[1].addr) {
        shutter_registers[1].val = dbg->val;
        return 0;
    }
    else if(dbg->reg == shutter_registers[2].addr) {
        shutter_registers[2].val = dbg->val;
        /*
        return ov9281_write_array(ov9281->client, shutter_registers);
                    bool s0 = SetRegister(0x3500,((val & 0xf0000) >> 16));
                    bool s1 = SetRegister(0x3501,((val & 0xff00) >> 8));
                    bool s2 = SetRegister(0x3502,(val & 0x00ff));
        */
        combined = ((u32)(shutter_registers[0].val) << 16) | ((u32)(shutter_registers[1].val) << 8) | (u32)(shutter_registers[2].val);

        if (pm_runtime_get(&client->dev) <= 0)
            return 0;

        ov9281_write_reg(client, 0x3208, OV9281_REG_VALUE_08BIT, 0x00);    /* Group 0 hold start */

        /* Write shutter value */
        ret = ov9281_write_reg(client, shutter_registers[0].addr, 3, combined);

        ov9281_write_reg(client, 0x3208, OV9281_REG_VALUE_08BIT, 0x10);    /* Group 0 hold end */
        ov9281_write_reg(client, 0x320B /*NB: B, not 8, in last digit*/, OV9281_REG_VALUE_08BIT, 0x00);    /* Manual launch on */
        ov9281_write_reg(client, 0x3208, OV9281_REG_VALUE_08BIT, 0xA0);    /* Delay launch group 0 */
        ...
        /* Note (automatic) type conversions. The register is only 8-bit, but ov7251_write_reg expects u32, and dbg->val is u64. */
        return ov9281_write_reg(client, dbg->reg, OV9281_REG_VALUE_08BIT, dbg->val);
    }
}
