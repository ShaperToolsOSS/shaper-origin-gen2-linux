...
/**
 * struct brcmf_auth_req_status_v2_le - external auth request with BSS info
 *
 * @version: version of the auth request event payload
 * @len: length of the auth request event payload
 * @flags: flags for external auth status
 * @peer_mac: peer MAC address
 * @ssid_len: length of ssid
 * @ssid: ssid characters
 * @pmkid: PMK identifier from firmware
 * @bss_info_le: optional BSS information for the target AP
 */
struct brcmf_auth_req_status_v2_le {
	__le16 version;
	__le16 len;
	__le16 flags;
	u8 peer_mac[ETH_ALEN];
